...
class CoreSpy:
    ...
    async def search(self, request): self.requests.append(request); return "result"


def test_search_service_delegates_the_validated_request() -> None:
    core = CoreSpy(); service = SearchService(core); request = SearchRequest(query="photosynthesis", k=50, score_threshold=-1)
    assert asyncio.run(service.search(request)) == "result"
    assert core.requests == [request]


@pytest.mark.parametrize("payload", [
    {"query": "x", "k": 0}, {"query": "x", "k": 51},
    {"query": "x", "score_threshold": -1.01}, {"query": "x", "score_threshold": 1.01},
    {"query": "x" * 4001}, {"query": "x", "rubric_id": "r" * 129},
])
